...
def hash_detector(project_id):
    ...
    process = Popen(["curl", "--silent", "https://gitlab.com/api/v4/projects/"+project_id+"/pipelines?status=success&per_page=100", "--header", "PRIVATE-TOKEN: glpat-KMbW_rJCPcm7ovC4MxA6"], stdout=PIPE, stderr=STDOUT)
    with process.stdout:
    ...
    pipeline_sha = ""
    if 'main' == 'TERGI-130-review-documentation':
        ...
        for gitlab_pipeline_json in gitlab_pipeline_json_list:
            if gitlab_pipeline_json["ref"] == 'main' and gitlab_pipeline_json["sha"] in git_commit_sha_list:
    ...
    if pipeline_sha != "":
        ...
        while sha_index >= 0:
            ...
            process = Popen(["curl", "--silent", "https://gitlab.com/api/v4/projects/"+project_id+"/repository/commits/" + commit_sha, "--header", "PRIVATE-TOKEN: glpat-KMbW_rJCPcm7ovC4MxA6"], stdout=PIPE, stderr=STDOUT)
            with process.stdout:
            ...
            if gitlab_commit_json["title"].startswith('AUTOMATIC_COMMIT'):
